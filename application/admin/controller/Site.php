                ...
                if ($result['type'] == 1) {
                    ...
                    if ($result['total'] > $result['need'] || $result['total'] == $result['need']) {
                        ...
                        Db::name('money')
                            ...
                            ->where('coin_name', 'UZF')
                            ->setInc('money', $result['total']);

                        Db::name('wallet')
                            ->data([
                                'us_id' => $result['user_id'],
                                'wa_num' => $result['total'],
                                'wa_type' => 2, 'wa_note' => '投票所获收益',
                                'add_time' => date('Y-m-d H:i:s')
                            ])->insert();
                    ...
                    //查询参与此次选举的所有系统服务商票数
                    $election = Db::name('election')
                        ->where('v_id', $id)
                        ->where('votes', '>=', $need_votes)
                        ->field('user_id,votes')
                        ->order('votes', 'desc')
                        ->limit(3)
                        ->select();
                    if (empty($election)) {
                        ...
                        //待替换的超级节点
                        $superNode = Db::name('user')
                            ->where('us_type', 4)
                            ->order('us_add_time', 'desc')
                            ->field('id')
                            ->limit(count($election))
                            ->select();
                        for ($i = 0; $i < count($election); $i++) {
                            ...
                            if (!empty($team)) {
                                ...
                                $list = Db::name('user')
                                    ->whereRaw("find_in_set($electionId,us_path)")
                                    ->field('id,us_path')
                                    ->select();

                                $result = Db::name('user')
                                    ->whereRaw("find_in_set($superId,us_path)")
                                    ->whereRaw("!find_in_set($electionId,us_path)")
                                    ->field('id,us_path')
                                    ->select();
                                foreach ($list as $k => $v) {
                                ...
                                $tempyh = Db::name('user')
                                    ->where('us_pid', $electionId)
                                    ->field('id')
                                    ->select();

                                Db::name('user')
                                    ->where('us_pid', $superId)
                                    ->update(['us_pid' => $electionId]);
                                foreach ($tempyh as $k => $v) {

                                    Db::name('user')
                                        ->where('id', $v['id'])
                                        ->setField('us_pid', $superId);
                                }
                                ...
                                Db::name('user')
                                    ...
                                    ->update([
                                        'us_type' => 3,
                                        'us_add_time' => date('Y-m-d H:i:s'),
                                        'us_path' => '0,' . $tempId['us_pid'],
                                        'us_pid' => $tempId['us_pid']
                                    ]);
