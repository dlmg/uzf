...
class Profit extends Common
{
    ...
    public function fb()
    {
        ...
        if (input('get.start') == "" && input('get.end')) {
            $map[] = ['add_time', '=<', input('get.end')];
        }
        if (input('get.start') && input('get.end')) {
            $map[] = ['add_time', 'between', array(input('get.start'), input('get.end'))];
        }
        if (input('get.coin_id')) {
            $map[] = ['coin_id', '=', input('get.coin_id')];
        }

        $result = Db::name('ex_order')
            ->alias('a')
            ->where($map)
            ->join('user b','a.from_id = b.id')
            ->join('user c','a.to_id = c.id')
            ->field('a.or_num,a.number,a.status,a.price,a.add_time,a.coin_id,b.us_nickname as sell,c.us_nickname as buy')
            ->paginate(10);
