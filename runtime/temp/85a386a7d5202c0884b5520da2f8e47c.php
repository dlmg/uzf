<?php /*a:3:{s:67:"D:\phpstudy_pro\WWW\uzf1\uzf\application\admin\view\user\index.html";i:1588902665;s:69:"D:\phpstudy_pro\WWW\uzf1\uzf\application\admin\view\public\_meta.html";i:1530329594;s:71:"D:\phpstudy_pro\WWW\uzf1\uzf\application\admin\view\public\_footer.html";i:1530790558;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<span class="l" style="margin: 10px">
			<a href="javascript:;" onclick="rank()" class="btn btn-primary radius">
			一键升降级</a>
		</span>
		...
		<tbody>
			<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				<tr class="text-c">
					...
					<td><select onchange="change(<?php echo htmlentities($vo['id']); ?>,$(this).val(),'us_type')">
						<option value="4" <?php if($vo['us_type'] == 4): ?>selected<?php endif; ?>>超级节点</option>
						<option value="3" <?php if($vo['us_type'] == 3): ?>selected<?php endif; ?>>系统服务商</option>
						<option value="2" <?php if($vo['us_type'] == 2): ?>selected<?php endif; ?>>商家</option>
						<option value="1" <?php if($vo['us_type'] == 1): ?>selected<?php endif; ?>>会员</option>
					</select>
					</td>
					<td>
						<?php if($vo['us_level'] == 1 and $vo['us_type'] == 1): ?>
						一级
						<?php elseif($vo['us_level'] == 2 and $vo['us_type'] == 1): ?>
						二级
						<?php elseif($vo['us_level'] == 3 and $vo['us_type'] == 1): ?>
						三级
						<?php elseif($vo['us_level'] == 4 and $vo['us_type'] == 1): ?>
						四级
						<?php else: ?>
						高级
						<?php endif; ?>
					...
					<?php if($vo['us_status'] == 1): ?>
						<a style="text-decoration:none" onClick="change(<?php echo htmlentities($vo['id']); ?>,0,'us_status')" href="javascript:;" title="停用"><span class="label label-success radius">已激活</span></a>
					<?php else: ?>
						<a style="text-decoration:none" onClick="change(<?php echo htmlentities($vo['id']); ?>,1,'us_status')" href="javascript:;" title="启用"><span class="label label-default radius">未激活</span></a>
					<?php endif; ?>
					</td>					
...
function rank() {
    layer.confirm('确定要进行升降级么？', {
        btn: ['确定', '取消']
    }, function(index, layero){
        $.ajax({
            type: "post",
            url: "<?php echo url('rank'); ?>",
            success: function(data) {
                layer.msg(data.msg);
                setTimeout("window.location=\"<?php echo url('User/index'); ?>\"",500);
            }
