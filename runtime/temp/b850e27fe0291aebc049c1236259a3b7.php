<?php /*a:1:{s:61:"D:\phpStudy\WWW\szsc\application\index\view\order\to_buy.html";i:1534406820;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
	...
	<body>
		<div>
			...
			<div class="body">
				<!--收货详情-->
				<div class="body_header" onclick="window.location.href='<?php echo url('user/myaddr'); ?>?type=<?php echo htmlentities($info['type']); ?>&pd_id=<?php echo htmlentities($info['id']); ?>'">
					<ul>
				...
				<div class="body_content">
					...
					<dl>
						<dt><img src="<?php echo htmlentities($info['pd_pic']['0']); ?>"/></dt>
						<dd>
							<ul>
								<li style="font-size: 0.24rem;" class="text_hidden"><?php echo htmlentities($info['pd_content']); ?></li>
								<li><p>颜色分类：<span><?php echo htmlentities($info['type']); ?></span></li>
								<li style="display: flex;"><p style="color: #CD1C1C;" >￥<?php echo htmlentities($info['pd_price']); ?></p> <p style="text-align: right;">✖1</p></li>
							</ul>
						</dd>						
					</dl>
				</div>
				...
				<div class="body_shopp">
					<label for=""><span>购买数量</span><p><input type="number" name="pd_num" id="pd_num" value="1" onchange="count()"></p></label>
				   	<label for=""><span>快递费</span><p><?php echo htmlentities($info['take_fee']); ?></p></label>
					<label for=""><span>买家留言</span><input type="text" id="or_remark" name="or_remark" placeholder="填写备注留言或者信息"/></label>
					<input type="hidden" name="" value="<?php echo htmlentities($info['pd_price']); ?>" id="pd_price">
					<input type="hidden" name="pd_id" value="<?php echo htmlentities($info['id']); ?>" id="pd_id">
					<input type="hidden" name="" value="<?php echo htmlentities($info['take_fee']); ?>" id="take_fee">
					<input type="hidden" name="addr_id" value="<?php echo htmlentities($addr['id']); ?>" id="addr_id">
				</div>
				</form>
			</div>			
			<div class="foot">
				<p>合计金额 :  ￥<span class="money"><?php echo htmlentities($info['allfee']); ?></span></p>
				<!-- <button >提交订单</button> -->
		...
		<script>
			...
			/*function subOrder(){
				(function(){
					//需要参数：地址addr_id 商品pd_id 数量pd_num 快递费take_fee 备注or_remark
					var addr_id = $('#addr_id').val();
					var pd_id = $('#pd_id').val();
					var pd_num = $('#pd_num').val();
					var take_fee = $('#take_fee').val();
					var or_remark = $('#or_remark').val();
					$.ajax({
						type: "post",
						url: "<?php echo url('order/subOrder'); ?>",
						data: {addr_id:addr_id,pd_id:pd_id,pd_num:pd_num,take_fee:take_fee,or_remark:or_remark},
			...
			function subOrder(){
			    $.post('<?php echo url("order/subOrder"); ?>',$('#login-form').serialize()).success(function(data){
			    	layer.msg(data.msg);
			    	if(data.code){
			    		$('#or_id').val(data.data);
			    	}
